        ...
        sp<AudioSessionInfo>& info, bool added)
{
    ...
    ALOGD("callback for event %d, added %d, session id: %d, stream: %d, flags %d, mask: %d, uid: %d",
     event, added, info->mSessionId, info->mStream, info->mFlags, info->mChannelMask, info->mUid);
    ...
    switch (ret) {
        ...
        case JNI_EDETACHED :
            if (gJvm->AttachCurrentThread(&env, NULL) < 0) {
                ALOGE("Failed to get the environment using AttachCurrentThread()");
                return;
            }
            ALOGD("Attached thread.");
            break;
        case JNI_EVERSION :
            // Cannot recover from this error
            ALOGE("JNI interface version 1.6 not supported");
            return;
        default :
            ALOGD("Failed to get the environment using GetEnv()");
            return;
    }
    ...
    if (ret == JNI_EDETACHED) {
        ALOGW("Also detaching from thread.");
...
JNIEXPORT jint JNICALL org_cyanogenmod_cmaudio_service_CMAudioService_listAudioSessions(JNIEnv *env, jobject thiz,
...
{
    android::gJvm = vm;  // cache the JavaVM pointer
    ALOGV("OnLoad");
    JNIEnv* env = 0;
